...
export interface PopupProps {
  ...
  // Auto-open settings
  autoOpen: boolean;
  autoOpenDelay: number; // delay in seconds
...
const defaultProps = {
  ...
  autoOpen: false,
  autoOpenDelay: 3, // 3 seconds default
...
export const Popup = (props: Partial<PopupProps>) => {
  ...
  const { closePopup, currentPopupIdOpen, openPopup, registerAutoOpenPopup } = useViewport();
  ...
  // Get popup open state from viewport context
  ...
  const { handleContainerClick, handleHover } = useEventHandling(events);
  ...
  // Auto-open popup logic - register with centralized manager (only in view-only mode)
  useEffect(() => {
    // Only auto-open in preview mode (not enabled) and when autoOpen is true
    if (!enabled && autoOpen && autoOpenDelay > 0) {
      console.log('🔥 Registering popup for auto-open:', { id, autoOpenDelay, enabled });
      registerAutoOpenPopup(id, autoOpenDelay);
    }
  }, [enabled, autoOpen, autoOpenDelay, id, registerAutoOpenPopup]);
  ...
  const handleCloseClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (enabled) {
      // In editor mode, deselect the node to hide the popup
      actions.selectNode([]);
    } else {
      // In preview mode, actually close the popup
      closePopup();
    }
  };

  const dynamicZIndex = zIndex.popupMain;
  ...
  // Create the popup content for modal
  const popupContent = (

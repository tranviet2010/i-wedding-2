...
interface UseButtonInteractionProps {
  id: string;
  enabled: boolean;
...
export const useButtonInteraction = ({
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
        // Clicked outside the button, exit child access mode
  ...
  const handleOverlayTouch = useCallback((e: React.TouchEvent) => {
    ...
    if (timeDiff < 500 && touchCount === 1) {
      ...
      // Handle single tap - select the button and enable drag mode
      if (enabled) {
  ...
  const handleOverlayClick = useCallback((e: React.MouseEvent) => {
    ...
    // Handle single click - select the button and enable drag mode
    if (clickCount === 0) {
      if (enabled) {
        setCurrentSelection(id);
        // Button is now selected and immediately draggable via craft.js connectors
      }
    }
